        ...
        protected void SmearDropDownList_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (SmearDropDownList.Text == "A")
                {
                    ...
                    s33 = "select sl_no ,date,Room_No, MAX(Air_activity) as air_activity from new_cdms WHERE (Air_activity != '' && Air_activity != 'BDL') && date != CURDATE() GROUP by date   order by date desc limit 1;";
                ...
                else if (SmearDropDownList.Text == "S")
                {
                    ...
                    string s34;
                    s34 = "select sl_no ,date,Room_No, MAX(contamination_level) as contamination_level from new_cdms WHERE (contamination_level != '' && contamination_level != 'BDL') && date != CURDATE() GROUP by date   order by date desc limit 1;";
